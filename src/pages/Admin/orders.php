...
$queryz = "SELECT COUNT(*) as unread_order FROM orders WHERE status = 'placed'";
...
$queryz1 = "SELECT COUNT(*) as unread_order FROM orders WHERE status = 'preparing'";
...
$queryzb = "SELECT COUNT(*) as unread_order FROM orders WHERE status = 'delivery'";
$result41b = $db->query($queryzb);

if ($result41b) {
    $row41b = $result41b->fetch_assoc();
    $unreadNotificationCountb = $row41b['unread_order'];
} else {
    $unreadNotificationCountb = 0; // Default to 0 if query fails
}
...
<head>
    ...
    <link rel="stylesheet" href="css/log5.css">
        ...
        <div class="row row-flex">
            ...
            <div class="col-sm-1 custom-width" style="height:100vh;">
                ...
                <div class="sidebar" style="height:100vh;">
                    ...
                    <a href="orders.php?logout=1" class="item">
            ...
            <div class="col-sm-11 wrap" style="padding:15px; height:100vh;">
                <div class="row">
                    ...
                    <div class="col-sm-12">
                        <div class="wrapper">
                            <h2><i class="fa-solid fa-receipt" style="margin-left:5px;"></i> Successful Orders</h2>
                            ...
                            if ($unreadNotificationCountb > 0) {
                                echo '<span class="notification-countb">' . $unreadNotificationCountb . '</span>';
                            }
                        ?>
                                 <a href="logs.php" class="btn btn-primary" style="margin-top:10px;"><i class="fa-solid fa-file"></i> New Orders</a>
                            ...
                            <hr>
                            <div class = "wrapper2" style="height:auto; overflow:auto; display: flex; flex-wrap: wrap; padding-left:20px; padding-top:20px;">
                            <?php
                            $servername = "localhost";
                            $username = "root";
                            $password = "";
                            $dbname = "ph_db";  
                            $conn = new mysqli($servername, $username, $password, $dbname);     
                            $sql = "SELECT * FROM success_orders";     
                            $result = $conn->query($sql);   
                            if ($result->num_rows > 0) {
                                while($row = $result->fetch_assoc()) {
                                    
                             $orderPlaced = $row['orderPlaced'];
                             $orderDelivered = $row['orderDelivered'];
                            ...
                            $time = $dateTime->format("h:i A");
                             $time1 = $dateTime1->format("h:i A");
                                    ...
                                    echo "<p class='card-text'>Delivered At: " . $time1. "</p>";
                                    echo "<div style='text-align:center;'><a href='order_details4.php?order_id=" . $row["orderID"]. "' class='btn btn-primary'>View Order</a></div>";
                                    echo "</div>";
                                    echo "</div>";
                                }
                            } else {
